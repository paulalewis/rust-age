...
impl YahtzeeSimulator {
    ...
    fn score_three_of_a_kind(dice_values: &[u8; N_VALUES]) -> u16 {
        for i in 0..N_VALUES {
            if dice_values[i] >= 3 {
                return YahtzeeSimulator::sum_of_values(dice_values);
            }
        }
        return 0;
    }
    ...
    fn score_chance(dice_values: &[u8; N_VALUES]) -> u16 {
        YahtzeeSimulator::sum_of_values(dice_values)
    }
        ...
        match action {
            YahtzeeAction::YahtzeeRollAction { selected } => {
                ...
                for _ in num_selected..N_DICE {
                    ...
                    dice_values[roll] = dice_values[roll] + 1;
                }
                rolls += 1;
            },
            YahtzeeAction::YahtzeeSelectAction { score_category } => {
                let score = match score_category {
                    YahtzeeScoreCategory::Ones => YahtzeeSimulator::score_ones(&dice_values),
                    YahtzeeScoreCategory::Twos => YahtzeeSimulator::score_twos(&dice_values),
                    YahtzeeScoreCategory::Threes => YahtzeeSimulator::score_threes(&dice_values),
                    YahtzeeScoreCategory::Fours => YahtzeeSimulator::score_fours(&dice_values),
                    YahtzeeScoreCategory::Fives => YahtzeeSimulator::score_fives(&dice_values),
                    YahtzeeScoreCategory::Sixes => YahtzeeSimulator::score_sixes(&dice_values),
                    YahtzeeScoreCategory::ThreeOfKind => YahtzeeSimulator::score_three_of_a_kind(&dice_values),
                    YahtzeeScoreCategory::FourOfKind => YahtzeeSimulator::score_four_of_a_kind(&dice_values),
                    YahtzeeScoreCategory::FullHouse => YahtzeeSimulator::score_full_house(&dice_values), 
                    YahtzeeScoreCategory::SmallStraight => YahtzeeSimulator::score_small_straight(&dice_values),
                    YahtzeeScoreCategory::LargeStraight => YahtzeeSimulator::score_large_straight(&dice_values),
                    YahtzeeScoreCategory::Yahtzee => YahtzeeSimulator::score_yahtzee(&dice_values),
                    YahtzeeScoreCategory::Chance => YahtzeeSimulator::score_chance(&dice_values),
                };
